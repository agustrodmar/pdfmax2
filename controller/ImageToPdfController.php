            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['images'])) {
                ...
                foreach ($_FILES['images']['tmp_name'] as $key => $tmpName) {
                    ...
                    if (getimagesize($tmpName) === false) {
                        echo "El archivo no es una imagen válida: " . htmlspecialchars($_FILES['images']['name'][$key]) . "<br>";
                        continue;
                    }

                    // Sanitizar el nombre del archivo
                    $safeFileName = preg_replace('/[^a-zA-Z0-9_\.-]/', '_', basename($_FILES['images']['name'][$key]));
                    $uniqueId = uniqid('IMG_', true); // Generar un ID único
                    $filePath = $uploadDir . $uniqueId . '_' . $safeFileName; // Establecer la ruta de destino con ID único
